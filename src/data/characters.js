// ─────────────────────────────────────────────────────────────
//  SHINKU — Character Roster (83 characters)
//  Stats are lore-accurate based on each character's feats.
//
//  Scale per stat:
//    55–65  → skilled human / early-series
//    66–75  → above human, low superhuman
//    76–85  → serious fighter, mid-tier
//    86–93  → elite / top-tier in their verse
//    94–99  → god-tier / pinnacle of their universe
//
//  Stats:
//    power        → raw destructive output
//    speed        → movement + reaction speed
//    defense      → durability + damage mitigation
...
export const getRandomCharacters = (count = 12) => {
  return [...ALL_CHARACTERS]
    ...
    .slice(0, Math.min(count, ALL_CHARACTERS.length))
    .map((c) => ({ ...c, hp: 1000 }));
};